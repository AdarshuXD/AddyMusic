...
from Addy.utils.database import get_assistant, get_authuser_names, get_cmode
from Addy.utils.decorators import ActualAdminCB, AdminActual, language
from Addy.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await Adarshu.stop_stream_force(message.chat.id)
    except:
        pass
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await Adarshu.stop_stream_force(chat_id)
        except:
            pass
...
async def close_menu(_, CallbackQuery):
    try:
        await CallbackQuery.answer()
        await CallbackQuery.message.delete()
        await CallbackQuery.message.reply_text("ᴊᴀᴋᴇ ᴍᴇʀᴇ ᴄʀᴇᴀᴛᴏʀ sᴇ ʙᴀᴀᴛ ᴋʀᴏ @SexyCoder",)
    except:
        pass
